...
```{r}
res_panaroo <- fit_tweedie(pa, tree)
res_roary <- fit_tweedie(rpa, tree)
```

```{r}
pp <- plot_pangenome_fits(list(panaroo=res_panaroo, roary=res_roary))
...
```{r}
comp <- compare_pangenomes(res_panaroo, res_roary, intercept = FALSE)
comp
```
